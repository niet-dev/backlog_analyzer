...
DEFAULT_MAPPING = mappings.ColumnMapping(
    TEST_SOURCE_COLUMNS, TEST_TARGET_COLUMNS)
...
class TestColumnRenamer:
    ...
    def test_sets_value_error_message(self):
        ...
        assert str(
            exception_info.value) == "One or more source columns is missing."
...
class TestAllRenamedColumnsExist:
    def test_returns_true_if_all_columns_exist(self, default_backlog):
        test_data = empty_dataframe(
            ["IGDB ID", "Game Name", "Completion Status"])

        assert default_backlog._all_renamed_columns_exist(test_data)

    def test_returns_false_if_a_column_is_missing_from_the_dataframe(
            self, default_backlog):
        test_data = empty_dataframe(
            ["IGDB ID", "Not Game", "Completion Status"])

        assert not default_backlog._all_renamed_columns_exist(test_data)

    def test_disregards_extra_columns(self, default_backlog):
        test_data = empty_dataframe(
            ["IGDB ID", "Another one", "Game Name", "This too", "Completion Status"])
...
class TestDropExtraColumns:
    ...
    def test_throws_key_error_if_column_is_missing(self):
        backlog = empty_backlog(
            ["ID", "Another one", "This too", "Completion"])
...
class TestGenerate:
    def test_renames_and_drops(self):
        backlog = empty_backlog(
            ["ID", "Another one", "Game", "This too", "Completion"])
